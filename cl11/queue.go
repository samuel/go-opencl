...
func (q *CommandQueue) EnqueueWaitForEvents(eventWaitList []*Event) error {
	return toError(C.clEnqueueWaitForEvents(q.clQueue, C.cl_uint(len(eventWaitList)), eventListPtr(eventWaitList)))
}
...
func (q *CommandQueue) EnqueueBarrier() error {
	return toError(C.clEnqueueBarrier(q.clQueue))
}
